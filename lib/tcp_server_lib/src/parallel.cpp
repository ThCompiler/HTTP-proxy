...
namespace prll {
    ...
    void Parallel::_balance() {
        while(true) {
            ...
            _in_balance.wait(lck,[this] { return _exit
            ...
            if(_tasks.size() > (size_t)(resize_coef*(double)_threads.size()) && _threads.size() < (size_t)_max_threads) {
                _threads.emplace_back(new Thread(_task_mutex, _tasks, _in_thread));
            } else {
                if(_threads.size() > (size_t)(resize_coef*(double)_tasks.size()) && _threads.size() > (size_t)1) {
                    _threads.back()->wait();
                    _threads.back()->force_join();
                    _threads.pop_back();
                }
